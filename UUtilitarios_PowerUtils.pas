...
begin
  ...
  begin
    Try
      Try
      {$Region 'Criar objeto de conexÃ£o com o banco e configura a conexÃ£o'}
        CoInitialize(nil);
        Alias := TAdoConnection.Create(Application);
        Alias.Connected      := False;
        //A conexÃ£o deve vir inicialmente fechada
        //Com xaCommitRetaining apÃ³s commitar ele abre uma nova transaÃ§Ã£o,
        //Com xaAbortRetaining  apÃ³s abordar ele abre uma nova transaÃ§Ã£o, custo muito alto.
        Alias.Attributes := [];
        Alias.CommandTimeout := 1;
        //Se o comando demorar mais de 1 segundos ele aborta
        Alias.ConnectionTimeout := 15;
        //Se demorar mais de 15 segundos para abrir a conexÃ£o ele aborta
        Alias.CursorLocation := clUseServer;
        //Toda informaÃ§Ã£o ao ser alterada sem commitar vai ficar no servidor.
        Alias.DefaultDatabase := '';
        Alias.IsolationLevel := ilReadUncommitted;
        //Quero saber os campos que ainda nÃ£o foram commitados tambÃ©m
        Alias.KeepConnection := True;
        Alias.LoginPrompt    := False;
        Alias.Mode           := cmRead;
        //Somente leitura
        ...
        Alias.Provider       := 'SQLNCLI11.1';
        Alias.Tag            := 1;
        //Para indicar que Ã© usado em VerificarCamposDaTabela
      ...
      {$Region 'Realiza consulta e escreve dados na tela'}
        ...
        with consultant do begin
          ...
          Open;
          {$Region 'Se retornar algo então retornar'}
            First;
            I := 0;
...
//              end);
    ...
    Except on E: Exception do
           begin
             ...
             Procedure
             begin
               Mensagem('Ocorreu o seguinte erro durante a execução: ' + ErrorMessage + FimLinhaStr + 
                        'Do seguinte script:' + FimLinhaStr + Concatenar(ScriptSQL));
             end);
           end;
    End;    // Pois o resto não precisa disso
    TThread.Queue(nil,
    Procedure
    begin
      Callback;
